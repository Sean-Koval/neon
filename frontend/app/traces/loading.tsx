/**
 * Traces List Loading State
 */

export default function TracesLoading() {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <div className="h-8 w-24 bg-gray-200 dark:bg-dark-700 rounded animate-pulse" />
          <div className="h-5 w-40 bg-gray-100 dark:bg-dark-800 rounded mt-2 animate-pulse" />
        </div>
        <div className="h-10 w-24 bg-gray-200 dark:bg-dark-700 rounded-lg animate-pulse" />
      </div>

      {/* Filters */}
      <div className="flex gap-4 animate-pulse">
        <div className="h-10 w-64 bg-gray-100 dark:bg-dark-800 rounded-lg" />
        <div className="h-10 w-32 bg-gray-100 dark:bg-dark-800 rounded-lg" />
        <div className="h-10 w-28 bg-gray-100 dark:bg-dark-800 rounded-lg" />
      </div>

      {/* Table skeleton */}
      <div className="card overflow-hidden">
        {/* Table header */}
        <div className="border-b bg-gray-50 dark:bg-dark-900 px-4 py-3 flex gap-4">
          {['Name', 'Status', 'Duration', 'Spans', 'Time'].map((col) => (
            <div
              key={col}
              className="h-4 bg-gray-200 dark:bg-dark-700 rounded animate-pulse flex-1"
            />
          ))}
        </div>
        {/* Table rows */}
        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
          <div key={i} className="border-b px-4 py-4 flex gap-4 animate-pulse">
            <div className="h-5 bg-gray-200 dark:bg-dark-700 rounded flex-1" />
            <div className="h-5 w-20 bg-gray-100 dark:bg-dark-800 rounded-full" />
            <div className="h-5 w-16 bg-gray-100 dark:bg-dark-800 rounded" />
            <div className="h-5 w-12 bg-gray-100 dark:bg-dark-800 rounded" />
            <div className="h-5 w-24 bg-gray-100 dark:bg-dark-800 rounded" />
          </div>
        ))}
      </div>
    </div>
  )
}
